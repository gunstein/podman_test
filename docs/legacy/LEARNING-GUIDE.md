# Historical per-container learning guide

Historical reference only; start with [the current Kube guide](../LEARNING-GUIDE.md).
The accepted baseline remains recoverable through `quadlet-reference-v1`.
Source paths and commands below describe that baseline, not current deployment.
