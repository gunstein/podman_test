...
On an Oracle Linux controller with active `fapolicyd`, verify the transferred
archive before trusting the exact Python source that Ansible must read:

```bash
sha256sum ~/todo-m13.5-test.tar.gz

sudo fapolicyd-cli --file add \
  "$HOME/todo-m13.5-test/scripts/todo_dr.py" \
  --trust-file todo-dr-source
sudo fapolicyd-cli --update
```

The build also creates `todo-m13-test.tar.gz.sha256`. Compare its value with one
received through a trusted channel; a checksum file copied together with a
modified archive does not establish authenticity. After a later source update,
use `--file update` for this path and then `--update`.
