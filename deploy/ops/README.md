# todo-ops: DR operations over plain SSH

`todo-ops` runs the DR and multi-host operations over plain `ssh` from the
...
On a controller where fapolicyd is active, todo-ops is itself project Python.
Trust its files once before the first run, from the extracted operations
package:

```bash
sudo sh deploy/scripts/trust-files.sh trust todo \
  "$PWD"/deploy/ops/todo_ops/*.py "$PWD"/deploy/installer/app_installer/*.py
```
...
python3 -m todo_ops --inventory initial.yaml preflight-standby
python3 -m todo_ops --inventory initial.yaml bootstrap-standby
python3 -m todo_ops --inventory initial.yaml replication-status
python3 -m todo_ops --inventory initial.yaml install-dr-tool
python3 -m todo_ops --inventory initial.yaml install-quarantine-tool
python3 -m todo_ops --inventory recovery.yaml deploy-promoted-application
python3 -m todo_ops --inventory recovery.yaml configure-backup
python3 -m todo_ops --inventory recovery.yaml rebuild-standby \
  --confirm-fenced "todo-primary is fenced" --confirm-reseed todo-primary
python3 -m todo_ops --inventory recovery.yaml cluster-status
```
