...
The playbook mounts each existing primary secret into a short-lived helper
container. Ansible suppresses the values with `no_log`, keeps them in memory and
sends them over SSH directly to `podman secret create` on standby. Pipelining is
enabled in the inventory so Ansible does not need normal module transfer files.
