...
On an Oracle Linux controller with active `fapolicyd`, trust the two exact
extracted source files that Ansible must interpret. The role task file contains
the pipelined inline Python installer, so `fapolicyd` can classify it as
executable content even though its extension is `.yml`:
...
Do not trust the complete role or extraction directory. The exact-file list
keeps the policy exception reviewable and ensures a changed source hash must be
